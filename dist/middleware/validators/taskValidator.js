...
export const validateTask = [
    ...
    body('createdBy')
        ...
        if (room.admin.equals(user._id)) {
            ...
            const memberRecord = room.members.find(member => member.user && member.user._id.equals(user._id) && member.role === 'host');
            if (memberRecord) {
                hasPermission = true;
            }
        }
    ...
    body('assignedTo')
        ...
        const roomMemberIds = room.members.map(member => member.user ? member.user._id.toString() : null).filter(Boolean);
