...
describe('Admin Authentication Tests', () => {
    ...
    beforeEach(() => {
        ...
        validAccessToken = jwt.sign({ id: mockAdmin._id, role: mockAdmin.role }, process.env.JWT_SECRET, { expiresIn: '1h' });
        validRefreshToken = jwt.sign({ id: mockAdmin._id, role: mockAdmin.role }, process.env.JWT_SECRET, { expiresIn: '7h' });
    });
    ...
    describe('Middleware verifiAdmin Tests', () => {
        ...
        test('Middleware với token của user không phải admin', async () => {
            const userToken = jwt.sign({ id: 'user123', role: 'user' }, process.env.JWT_SECRET, { expiresIn: '1h' });
            const response = await request(app)
                .get('/api/admin/protected')
        ...
        test('Middleware với token đã hết hạn', async () => {
            const expiredToken = jwt.sign({ id: mockAdmin._id, role: 'admin' }, process.env.JWT_SECRET, { expiresIn: '0s' });
...
//# sourceMappingURL=adminAuth.test.js.map