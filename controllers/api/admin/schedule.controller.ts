...
export const createSchedule = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const { title, description, room, participants, startTime, endTime } = req.body as ScheduleBody;
    ...
    let processedParticipants: string[] = [];
...
export const updateSchedule = async (req: Request<{ id: string }, {}, ScheduleBody>, res: Response) => {
  try {
    const scheduleId = req.params.id;
  ...
  export const updateSchedulePartial = async (
    req: Request<{ id: string }, {}, Partial<ScheduleBody>>,
    res: Response
  ) => {
    try {
      const scheduleId = req.params.id;
      ...
      const updates: Partial<ScheduleBody & { updatedAt: Date }> = { ...req.body };
      ...
      if (updates.startTime) {
        ...
        updates.startTime = startTimeDate as any;
      }
      if (updates.endTime) {
        ...
        updates.endTime = endTimeDate as any;
      }

      // Kiểm tra tính hợp lệ của startTime và endTime
      if (updates.startTime && updates.endTime) {
        if ((updates.endTime as Date) < (updates.startTime as Date)) {
          return res.status(400).json({
            success: false,
      ...
      if (updates.startTime && schedule.endTime) {
        if ((updates.startTime as Date) > schedule.endTime) {
          return res.status(400).json({
            success: false,
      ...
      if (updates.endTime && schedule.startTime) {
        if ((updates.endTime as Date) < schedule.startTime) {
          return res.status(400).json({
            success: false,
...
export const getSchedulesByRoom = async (req: Request<{ roomId: string }, {}, {}, { status?: string; page?: string | number }>, res: Response) => {
  try {
